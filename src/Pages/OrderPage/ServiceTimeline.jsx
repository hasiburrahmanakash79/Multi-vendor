import { MapPin, MessageCircle, Calendar, DollarSign, User, Mail } from 'lucide-react';
...
export default function ServiceTimeline() {
  ...
  const additionalTotal = orderData.additionals?.reduce((sum, item) => sum + parseFloat(item.price || 0), 0) || 0;
  ...
  return (
    <div className="min-h-screen py-8 container mx-auto mt-16 md:mt-0 px-4">
      <div className="mb-6">
        <Link to="/manage-orders" className="text-purple-600 hover:text-purple-700 font-medium flex items-center gap-2">
      ...
      <div className="relative h-48 sm:h-64 mb-8">
        <img 
          src={orderData.service?.cover_photo || "https://images.unsplash.com/photo-1519225421980-715cb0215aed?w=2070&q=80"} 
          alt={orderData.service?.title}
          className="w-full h-full object-cover rounded-2xl"
          onError={(e) => e.target.src = "https://images.unsplash.com/photo-1519225421980-715cb0215aed?w=2070&q=80"}
        />
        ...
        <div className="absolute top-4 right-4">
          <span className={`px-4 py-2 rounded-full text-sm font-semibold ${
            orderData.status === 'Active' ? 'bg-blue-500 text-white' :
            orderData.status === 'Completed' ? 'bg-green-500 text-white' :
            orderData.status === 'Pending' ? 'bg-yellow-500 text-white' :
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-4">
            <div>
              ...
              <p className="font-semibold text-green-600">{orderData.paid ? 'Paid ✓' : 'Unpaid'}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Payment Method</p>
              <p className="font-semibold text-gray-800">{orderData.payment_method}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                ...
                />
                <div>
                  <h4 className="font-semibold text-gray-800">{orderData.user?.full_name}</h4>
              ...
              <div className="space-y-3">
                {orderData.additionals.map((additional, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <span className="font-semibold text-gray-800">${parseFloat(additional.price).toFixed(2)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="font-medium text-gray-800 mb-1">Event Date & Time</h4>
                  <p className="text-gray-600">{orderData.event_date} at {orderData.event_time}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
            ...
            <div className="space-y-3">
              <div className="flex justify-between py-2">
                ...
                <span className="font-medium text-gray-800">${baseAmount.toFixed(2)}</span>
              </div>
              
              {orderData.additionals?.map((additional, index) => (
                <div key={index} className="flex justify-between py-2">
                  <span className="text-gray-600">{additional.title}</span>
                  <span className="font-medium text-gray-800">${parseFloat(additional.price).toFixed(2)}</span>
                </div>
              ))}
              ...
              <div className="flex justify-between py-2 bg-purple-50 -mx-6 px-6 rounded">
                ...
                <span className="font-bold text-purple-600 text-lg">${totalAmount.toFixed(2)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
            <div className="mb-6 space-y-3">
              <div className="flex justify-between items-center bg-amber-50 p-4 rounded-lg border border-amber-200">
                ...
                <span className="text-orange-600 font-semibold">{orderData.event_date}</span>
              </div>
