      ...
      <div className="bg-white rounded-lg w-1/2">
        ...
        <div className="flex items-center justify-between p-4 border-b">
          <div className="flex items-center">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Create Your Services</h3>
            </div>
          </div>
          <button
        ...
        <div className="p-4 space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-900 mb-2">
              Search Tags <span className="text-gray-500">(Enter relevant tags below and enhance)</span>
            </label>
            <input
              type="text"
              name="tags"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-2">Available time</label>
              <input
                type="text"
                name="time"
                value={formData.time}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-900 mb-2">
              Cover Photo <span className="text-red-500">(Image or video <span className="underline cursor-pointer" onClick={showSizeLimits}>see size limits</span>)</span>
            </label>
            <div 
            ...
            >
              {coverPreview ? (
                coverType.startsWith('image') ? (
                  <img src={coverPreview} alt="Cover preview" className="max-w-full max-h-40 mx-auto object-contain" />
                ) : (
                  <video src={coverPreview} controls className="max-w-full max-h-40 mx-auto" />
                )
              ) : (
                <>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-900 mb-2">
              Gallery <span className="text-red-500">(Image or video 1000 x 1000 max size 10MB)</span>
            </label>
            <div className="grid grid-cols-4 gap-3">
              {Array.from({ length: 3 }).map((_, index) => (
                <div key={index} className="border-2 border-dashed border-gray-300 rounded-lg aspect-square flex items-center justify-center bg-gray-50">
                  {galleryPreviews[index] ? (
                    galleryPreviews[index].type.startsWith('image') ? (
                      <img src={galleryPreviews[index].url} alt={`Gallery ${index + 1}`} className="w-full h-full object-cover rounded-lg" />
                    ) : (
                      <video src={galleryPreviews[index].url} controls className="w-full h-full object-cover rounded-lg" />
                    )
                  ) : (
                    <div className="flex flex-col items-center justify-center h-full text-center">
                      ...
                      <span className="text-xs text-gray-500">Image or Video</span>
                    </div>
                  )}
                </div>
              ))}
              <div 
                className={`border-2 border-dashed border-gray-300 rounded-lg aspect-square flex items-center justify-center bg-gray-50 cursor-pointer hover:bg-gray-100 ${galleryPreviews.length >= 3 ? 'opacity-50 cursor-not-allowed' : ''}`}
                onClick={() => {
        ...
        <div className="p-4 border-t bg-gray-50">
          <button
            ...
            className="w-full py-3 bg-purple-500 text-white rounded-md hover:bg-purple-600 transition-colors font-medium"
          >
            Publish Service
          </button>
        </div>
